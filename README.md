Installing StrataVane VPNStrataVane requires admin/root privileges to run (OpenVPN needs to modify network interfaces and routing tables). Pick your OS below.   Arch-based (CachyOS, Arch, Manjaro, EndeavourOS)Bashsudo pacman -Syu
...
cd YOUR_REPO
sudo python3 stratavaneVPN.py
BSD-based (FreeBSD)Package names can shift slightly between FreeBSD releases — if a name below doesn't resolve, run pkg search pyqt5 / pkg search beautifulsoup to find the current one for your version.Bashsudo pkg update
